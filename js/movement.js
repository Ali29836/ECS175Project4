...
class OrbitMovement
{
    ...
    updateMouse( )
    {
        ...
        if ( Input.isMouseDown( 0 ) && Input.mousex != -Infinity && Input.mousey != Infinity)
        {
            rotation = vec3.fromValues( -Input.getMouseDy( ) * this.speed, Input.getMouseDx( ) * this.speed, 0 )
        }

        if ( Input.isMouseDown( 2 ) && Input.mousex != -Infinity && Input.mousey != Infinity)
        {
...
class RaycastMovement {
    ...
    updateSelection()
    {
        if (Input.isMouseClicked( 0 ) && Input.mousex != -Infinity && Input.mousey != Infinity) {
    ...
    updateTransformation()
    {
        ...
        if ( Input.isMouseDown( 1 ) && Input.mousex != -Infinity && Input.mousey != Infinity)
        {
            let s = Input.getMouseDy( ) * this.speed * 0.01
            scale = vec3.add(scale, scale, vec3.fromValues( s, s, s ))
        }
